...
def kappa_from_cm(cm):
    """由 2x2 混淆矩阵计算 Cohen's Kappa（论文中的 KC）。"""
    cm = cm.astype(np.float64)
    total = cm.sum()
    po = np.trace(cm) / total
    pe = np.sum(cm.sum(1) * cm.sum(0)) / (total * total)
    return float((po - pe) / (1.0 - pe + 1e-12))


def scores_from_meter(meter):
    """把 ConfuseMatrixMeter 的输出整理成论文同口径指标字典（百分比由调用方乘）。"""
    s = meter.get_scores()
    return {
        'OA': float(s['acc']),
        'mIoU': float(s['miou']),
        'IoU_background': float(s['iou_0']),
        'IoU_change': float(s['iou_1']),
        'Recall_change': float(s['recall_1']),
        'Precision_change': float(s['precision_1']),
        'F1_change': float(s['F1_1']),
        'Recall_background': float(s['recall_0']),
        'KC': kappa_from_cm(meter.sum),
    }
